...
#'''In Galactic coodinates'''
#Sun = (0, 8.33, 0)
#psr1713 = (, z)

#print Delta(np.array([float(pf.PX[0])]), np.array([float(pf.SINI[0])]), np.array([float(pf.PAASCNODE)]), np.array([float(M1(pf))]), np.array([float(pf.M2[0])]), np.array([float(pf.PB)*secperday]), np.array([float(pf.E[0])]), np.array([1.]))


import numpy.random as npr
#fig = plt.figure()
#ax = fig.add_subplot(111)
#x = npr.randn(100) * np.pi * 2
#x %= (np.pi*2)
#ax.plot(x, [ Xi(y) for y in x], 'bo')
#plt.show()
