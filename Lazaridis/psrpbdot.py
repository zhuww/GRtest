...
def Pbdot_GW(pf):
    ...
    if pf.SINI == 'KIN':
        fac3 = cos(float(pf.KIN[0])*pi/180)*float(pf.KIN[1])
    else:
...
if __name__ == '__main__':
    ...
    pb_exc = (float(pf.PBDOT[0]) - Gal[0] - Shl[0] - GW[0]), sqrt(float(pf.PBDOT[1])**2 + Gal[1]**2 + Shl[1]**2 + GW[1]**2)*2
    print "PBdot - PBdot(Galaxtic) - PBdot(shlkovskii) - PBdot(GW), PBdot_exc_err*2:",
    print SF(pb_exc)   
    print "PBdot , PBdot(Galaxtic) + PBdot(shlkovskii) + PBdot(GW):",
    print pf.PBDOT[0] , Shl[0]+Gal[0]+GW[0]
    print "PBdot_excess/PB --> Gdot/G:",

    print SF((pb_exc[0]/float(pf.PB[0])/secperday, pb_exc[1]/float(pf.PB[0])/secperday))
